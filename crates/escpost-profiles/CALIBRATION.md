...
`calibration-job.hex` is the one comprehensive ESC/POS stream used to calibrate
every physical printer profile. Keeping the bytes shared makes printer
differences visible: the selected profile is the only rendering input that
changes.
...
cases remain under `crates/escpost-render/tests/cases/` because they are easier
to diagnose when one command or interaction fails.
