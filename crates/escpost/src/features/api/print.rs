...
struct JsonPrintBody {
    printer: String,
    /// Base64 of the exact ESC/POS bytes. A caller already holding bytes
    /// should post octet-stream instead.
    data: String,
}
...
/// Reduce either content type to a printer name and bytes, refusing anything
/// ambiguous. Pure, so the branches are unit tests rather than round trips.
...
/// This printer's lock, created on first use. Keyed by name because two
/// printers may print at once and one printer may not: a USB device can be
/// claimed once, and a RAW TCP printer is usually single-session.
