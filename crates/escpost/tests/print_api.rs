...
fn a_remote_page_origin_is_refused() {
    ...
    let response = guarded_probe(port, Some("https://evil.example"));
    stop(&mut child);
...
fn the_extension_and_local_callers_are_accepted() {
    ...
    let extension = guarded_probe(
        port,
        Some("chrome-extension://cnifebiebidolpmlmgcghpopggfcklmc"),
    );
    let opaque = guarded_probe(port, Some("null"));
    let absent = guarded_probe(port, None);
    stop(&mut child);

    for response in [&extension, &opaque, &absent] {
        assert_ne!(response_status(response), "HTTP/1.1 403 Forbidden");
    }
}

#[test]
fn the_listing_reports_transport_connection_and_a_resolvable_profile() {
...
",
    ...
    wait_until_listening(&mut child, port);
    let response = http_get(port, "/api/printers/list");
    stop(&mut child);

    assert_eq!(response_status(&response), "HTTP/1.1 200 OK");
    let body: serde_json::Value =
        serde_json::from_slice(response_body(&response)).expect("the listing should answer JSON");
    let printers = body["printers"]
        .as_array()
        .expect("the listing returns an array under `printers`");
    assert_eq!(printers.len(), 2);

    let counter = printers
        .iter()
        .find(|printer| printer["name"] == "counter")
        .expect("the USB printer should be listed");
    assert_eq!(counter["transport"], "usb");
    assert_eq!(counter["profile"], "TM-T88II");
    assert_eq!(counter["connection"]["vendor_id"], 0x04b8);

    let kitchen = printers
        .iter()
        .find(|printer| printer["name"] == "kitchen")
    ...
    assert_eq!(kitchen["connection"]["host"], "192.0.2.50");
    assert_eq!(kitchen["connection"]["port"], 9100);
...
#[test]
fn the_listing_is_ordered_so_the_first_entry_is_the_obvious_default() {
...
",
    ...
    let mut child = start_api_with_config(port, &config);
    wait_until_listening(&mut child, port);
    let listed = http_get(port, "/api/printers/list");
    stop(&mut child);

    assert_eq!(response_status(&listed), "HTTP/1.1 200 OK");
    let body: serde_json::Value =
        serde_json::from_slice(response_body(&listed)).expect("the listing should answer JSON");
    let printers = body["printers"]
        .as_array()
        .expect("an array under `printers`");

    // There is no default endpoint. The ordering is the contract instead:
    // connected before unavailable, then by name, so a client can take the
    // first entry and get the same answer every time. Both of these are
    // unavailable, so the tie breaks on name and alpha comes before zulu.
    assert_eq!(printers[0]["name"], "alpha");
    assert_eq!(printers[1]["name"], "zulu");

    std::fs::remove_dir_all(directory).expect("the test directory should be removable");
}

#[test]
fn the_listing_is_empty_rather_than_an_error_when_none_is_configured() {
    ...
    let body: serde_json::Value =
        serde_json::from_slice(response_body(&response)).expect("the listing should answer JSON");
    // No printers is a fact about the machine, not a failure of the request.
    assert!(
        body["printers"]
            .as_array()
            .expect("an array under `printers`")
...
fn pinning_an_extension_id_refuses_every_other_extension() {
    ...
    let allowed = guarded_probe(port, Some(&format!("chrome-extension://{pinned}")));
    let refused = guarded_probe(
        port,
    ...
    // may: nothing here requires an extension to exist at all.
    let local = guarded_probe(port, None);
    stop(&mut child);

    assert_ne!(response_status(&allowed), "HTTP/1.1 403 Forbidden");
    assert_eq!(response_status(&refused), "HTTP/1.1 403 Forbidden");
    assert_ne!(response_status(&local), "HTTP/1.1 403 Forbidden");
}
...
fn the_api_makes_no_outbound_connection_while_serving() {
    ...
    let _ = http_get(port, "/health");
    let _ = http_get(port, "/api/printers/list");
    let _ = http_get(port, "/api/printers/list");
    let _ = http_request(
        port,
        "POST",
...
/// A request the guard has to judge before anything else can. The printer does
/// not exist, so an accepted caller gets a typed 404 and a refused one gets
/// 403: the distinction under test is which of the two comes back.
fn guarded_probe(port: u16, origin: Option<&str>) -> Vec<u8> {
    let mut headers = vec!["Content-Type: application/json".to_owned()];
    if let Some(value) = origin {
        headers.push(format!("Origin: {value}"));
    }
    http_request(
        port,
        "POST",
        "/api/print",
        &headers,
        br#"{"printer":"nothing-by-this-name","data":"G0A="}"#,
    )
}
